...
2.  **Construct Gmail Query for Email Confirmation:**
    *   Create a Gmail search query string. This query MUST include:
        ...
        *   For online purchases or services (e.g., from Bokus, Amazon, or digital subscriptions):
            *   If the keywords from `Raw Description` are very specific vendor names (e.g., "Apple", "Spotify", "Netflix"), prioritize these keywords. Adding generic terms like "receipt" or "invoice" might be unnecessary or even counterproductive.
            *   If the keywords are more generic (e.g., "Online Store", "Digital Service"), or if you need to find itemized details, then consider adding terms like "order confirmation", "receipt", "your order", "invoice", or "order details" to the query.
        *   Date constraints using `after:` and `before:`. Set a narrow window around the transaction `Date`, typically +/- 1 days (e.g., if Date is 2025-05-01, use `after:2025/04/30 before:2025/05/02`). Format dates as YYYY/MM/DD for Gmail search.
